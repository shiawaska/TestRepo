...
constructor(private httpclient: HttpClient) { }
  ...
  private handleError(error: HttpErrorResponse) {
    ...
    return throwError(() => new Error('Something bad happened; Check planets service.'));
  }


  getPlanetsDropdown(): Observable<PlanetDropDownDto[]> {
    return this.httpclient.get<requestResult<PlanetDropDownDto[]>>('http://localhost:5125/api/Planet/dropdown').pipe(
  ...
  addPlanet(planet: PlanetFormDto): Observable<number> {
    return this.httpclient.post<requestResult<number>>('http://localhost:5125/api/Planet', planet).pipe(map(response => response.data),
      catchError(this.handleError)
    );
  }
  updatePlanet(planet: PlanetFormDto, id: number): Observable<number> {
    return this.httpclient.put<requestResult<number>>(`http://localhost:5125/api/Planet/${id}`, planet).pipe(map(response => response.data),
      catchError(this.handleError)
    );
  }
  deletePlanet(id: number): Observable<number> {
    return this.httpclient.delete<requestResult<number>>(`http://localhost:5125/api/Planet/${id}`).pipe(map(response => response.data),
      catchError(this.handleError)
    );
  }
